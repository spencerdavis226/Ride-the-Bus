  ...
  return (
    <div className="space-y-3">
      {names.map((name, index) => (
        <label key={index} className="flex items-center gap-2 rounded-lg bg-white/[0.08] p-2 ring-1 ring-white/10">
          <span className="w-8 text-center text-sm text-[#f5d99b]/75">{index + 1}</span>
          <input
            value={name}
            onChange={(event) => updateName(index, event.target.value)}
            className="min-h-11 flex-1 bg-transparent text-base font-semibold outline-none placeholder:text-white/35"
            placeholder={`Player ${index + 1}`}
          />
          <button
            aria-label={`Remove ${name || `Player ${index + 1}`}`}
            className="grid h-10 w-10 place-items-center rounded-md text-white/60 disabled:opacity-25"
