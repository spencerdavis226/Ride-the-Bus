  ...
  return (
    <div className="glass-panel rounded-xl p-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-[var(--rtb-accent)]">{dealSubphaseLabels[deal.subphase]}</p>
          <h2 className="mt-1 text-3xl font-bold">{player.name}</h2>
          <p className="mt-2 text-sm text-[var(--rtb-text-muted)]">{promptForSubphase(deal.subphase)}</p>
        </div>
        <div className="shrink-0 rounded-lg bg-[var(--rtb-surface-soft)] px-3 py-2 text-right ring-1 ring-[var(--rtb-border)]">
          <div className="text-lg font-bold">{player.hand.length}/4</div>
          <div className="text-xs text-[var(--rtb-text-muted)]">{shoeCount} in shoe</div>
        </div>
      </div>
      {deal.lastAssignment && (
        <div className="mt-4 rounded-lg bg-[var(--rtb-surface-soft)] p-3">
          <span className="font-semibold text-[var(--rtb-text)]">{deal.lastAssignment.playerName}</span>
          <span className="text-[var(--rtb-text-muted)]">: </span>
          <span className="text-[var(--rtb-accent)]">
