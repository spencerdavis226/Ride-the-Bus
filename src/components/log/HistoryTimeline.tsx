...
export function HistoryTimeline({ cards }: { cards: TableCard[] }) {
  ...
  if (!flipped.length) {
    return <p className="text-sm text-cream/60">No table cards flipped yet.</p>;
  }
  return (
      ...
      {flipped.map((card) => (
        <div key={card.id} className="rounded-lg bg-white/[0.07] p-3">
          <div className="flex items-center justify-between">
            ...
            <span className={card.card.color === 'red' ? 'text-history-red' : 'text-cream'}>
              {card.card.rank} {suitGlyphs[card.card.suit]}
            </span>
          </div>
          <div className="mt-2 space-y-1 text-sm text-cream/72">
